package org.cloudbus.cloudsim.test;
...
import org.cloudbus.cloudsim.energy.RenPowerBroker;
import org.cloudbus.cloudsim.energy.RenPowerCloudlet;
import org.cloudbus.cloudsim.energy.RenPowerVm;
import org.cloudbus.cloudsim.energy.RenPowercenter;
...
public class NewTest2 {
	...
	private static List<RenPowercenter> DatacenterList = new ArrayList<RenPowercenter>();
	private static List<RenPowerBroker> RenPowerBrokerList = new ArrayList<RenPowerBroker>();//接收传出的信息(模拟完毕)
//	private static RenPowerBroker broker ;
	private static List<RenPowerCloudlet> allcloudletList = new ArrayList<RenPowerCloudlet>();//根本用不上 用这个玩意submitcloudletlist之后就没有卵用了  竟然才看出来。。
	private static List<RenPowerVm> allvmlist = new ArrayList<RenPowerVm>();//恐怕用不上这个吧。不，有点用。。。帮忙记录vm的数量。。然后分发给vm id...
	private static ObjectInputStream ois = null;
	
	private static List<RenPowerVm> createVM(int userId, int nb_vm ,int type, String targetDatacenter) {

		List<RenPowerVm> currentList = new ArrayList<RenPowerVm>();
		...
		for(int i=0;i<nb_vm;i++){
			RenPowerVm vm = new RenPowerVm(allvmlist.size(), userId, mips, pesNumber, ram, bw, size, vmm, new CloudletSchedulerSpaceShared(),targetDatacenter);//���space��time��
			allvmlist.add(vm);
			currentList.add(vm);
		}
		return currentList;
...
//		System.out.println(ois);
		...
		try {
			...
			createDatacenter();
			for(RenPowercenter datacenter : DatacenterList){
				datacenter.setDisableMigrations(true);//别忘了。
			}
			
			RenPowerBroker broker = null;
			try {
				broker = new RenPowerBroker("broker_"+RenPowerBrokerList.size());
			} catch (Exception e) {
				e.printStackTrace();
			}
			RenPowerBrokerList.add(broker);//内部类可以调用外部类中的private static
	
			
			//Create VMs and Cloudlets and send them to broker
			
			List<RenPowerVm> vmlist = new ArrayList<RenPowerVm>();
			List<RenPowerCloudlet> missionList = new ArrayList<RenPowerCloudlet>();
			...
			for(int i = 0 ; i < 1 ; i ++){
				...
				List <RenPowerCloudlet> list = new ArrayList<RenPowerCloudlet>();
				...
				//测试用例的话，初始为模拟开始的时间。
				RenPowerCloudlet cloudlet = new RenPowerCloudlet(allcloudletList.size(), length, pesNumber, fileSize, outputSize, utilizationModel, utilizationModel, utilizationModel, new Date(CloudSim.getSimulationCalendar().getTimeInMillis()));
...
//			greedy();
			...
			List<Cloudlet> newList = RenPowerBrokerList.get(0).getCloudletReceivedList();
			for(int i=1 ; i < RenPowerBrokerList.size(); i++)//查看源代码发现。其实就是cloudlet全都被吞进去，又被吐出来，只不过中间加了一堆时间信息。好比喻。。(逃
				newList.addAll(RenPowerBrokerList.get(i).getCloudletReceivedList());
			//按照请求时间排序
			newList.sort(new Comparator<Cloudlet>(){

				@Override
				public int compare(Cloudlet o1, Cloudlet o2) {
					return ((RenPowerCloudlet)o1).getRequestDate().compareTo(((RenPowerCloudlet)o2).getRequestDate());
				}
			...
			for(RenPowercenter center : DatacenterList){
			...
			for(Cloudlet cloudlet : newList){
				System.out.println(((RenPowerCloudlet)cloudlet).getWaitTime()+"..."+((RenPowerCloudlet)cloudlet).getRequestDate());
			}
	...
	private static RenPowercenter createCenter(String name){
		...
		RenPowercenter datacenter = null;
		try {
			datacenter = new RenPowercenter(name, characteristics, new PowerVmAllocationPolicySimpleWattPerMipsMetric(hosts), storageList, 0.01);
//			RenPowercenter center = new RenPowercenter(s, characteristics, new VmAllocationPolicySimple(hosts), storageList, 0);
//			System.out.println(center.getPower());
	...
	private static List<RenPowerCloudlet> createCloudletList(int user_id, int nb_cloudlet, Date date){
		...
		for(int i = 0 ; i < nb_cloudlet ; i ++){
			...
			UtilizationModel utilizationModel = new UtilizationModelFull();
			//测试用例的话，初始为模拟开始的时间。
			RenPowerCloudlet cloudlet = new RenPowerCloudlet(allcloudletList.size(), length, pesNumber, fileSize, outputSize, utilizationModel, utilizationModel, utilizationModel, date);
		...
		public void run() {
			...
			while(true){
				...
				for(int i = 0 ; i < num_user ; i ++){
					...
					vmlist.sort(new Comparator<RenPowerVm>(){
						
						@Override
						public int compare(RenPowerVm o1, RenPowerVm o2) {
							return (int)((o1.getCurrentRequestedMaxMips()) - (o2.getCurrentRequestedTotalMips())) ;
						}
						
					});
					missionList.sort(new Comparator<RenPowerCloudlet>(){
						
						@Override
						public int compare(RenPowerCloudlet o1, RenPowerCloudlet o2) {
